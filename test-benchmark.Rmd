...
model_name="BUMMER"
## note: use vb=TRUE for large datasets, vb=FALSE is more accurate (uses MCMC)
## for real comparisons of model performance, use vb=FALSE
...
## note: use vb=TRUE for large datasets, vb=FALSE is more accurate (uses MCMC)
## for real comparisons of model performance, use vb=FALSE
MVGP=make_CV(y=y, X=X, model_name=model_name, kfold=2, 
               n_samples=500, n_grid=500,
               algorithm="fullrank",
               pooled=FALSE, vb=FALSE)

## pull out predictions and reformat output
X_pred_MVGP <- MVGP$X_pred
MVGP$mu <- apply(X_pred_MVGP, 1, mean)
MVGP$sd <- apply(X_pred_MVGP, 1, sd)
MVGP$X_pred <- NULL
